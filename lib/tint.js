...
var tint = module.exports = function(png, options) {
    ...
    var opacity = 'opacity' in options ? +options.opacity / 100 : 100;
    ...
    while (i < png.length) {
        ...
        if (type === 'PLTE') {
            ...
            if (opacity === 100) return png;
        // Apply opacity adjustment.
        } else if (palette && type === 'tRNS') {
            // If there are more entries in the palette than the tRNS
            // chunk length there are implicit A:255 values. Create a new
            // Buffer of increased length and copy the prior and latter chunks
            // leaving room for a tRNS chunk that matches palette length.
            if (palette > length) {
                var adjusted = new Buffer(png.length + (palette - length));
                png.copy(adjusted, 0, 0, i + length);
                png.copy(adjusted, i + palette + 4, i + length + 4);
                png = adjusted;
            }

            for (var entry = 0; entry < palette; entry += 1) {
                var val = (entry < length ? png[i + entry] : 255) * (opacity/100) | 0;
                png[i + entry] = val > 255 ? 255 : val;
            }

            // Update chunk length
            adjusted.writeUInt32BE(palette, i - 8);

            // Update CRC
            var crc = crc32(adjusted.slice(i - 4, i + palette));
            // Don't use buffer copy because it fails in node 0.4 due to
            // different instances of the Buffer object...
            png[i + palette] = crc[0];
